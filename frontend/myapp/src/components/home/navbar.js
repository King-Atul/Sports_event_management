...
// }, [dropdown])
   ...
   return (
    <div className="w-full h-24 bg-[#171717] py-2 shadow-md shadow-gray-400 fixed top-0 z-10  h-30/">
      <div className="container mx-auto p-2">
        <div className="flex flex-row justify-between items-center">
          <div className="flex flex-row justify-start items-center gap-2">
            <img src="logo1.jpg" alt="logo" className="h-10 w-10"></img>
            <h1 className="text-white font-[Lobster] font-light text-xl content-center">Sports Organization</h1>
          </div>
          <div className="flex flex-row justify-center items-center gap-6">
            {
                  Links.map((link)=>(
                    <li key={link.name} className='list-none'>
                      <a href={link.link} className='text-white text-xl hover:text-blue-500 hover:font-semibold duration-100 font-[Poppins] ease-in-out'>{link.name}</a>
                    </li>
                  ))
              }
          </div>
